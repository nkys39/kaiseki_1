...
本システムでは、**事前に用意されたCAD図面（既存地図）**と**SLAMアルゴリズム**を組み合わせて、ロボットの自己位置を推定しています。これは一般的なSLAM（地図構築と自己位置推定を同時に行う）とは異なり、**既存地図に対するスキャンマッチング**が主要な処理となります。
...
LRFで取得した点群データを、既存地図上の障害物位置と照合することで自己位置を推定します。
